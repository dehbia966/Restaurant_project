...
import os,time
from fonctionGit import ClientInfo
from fonctionGit import AjoutFournisseur,AjoutPersonnel,SupprimerPersonnel,passer_reservation
import builtins
...
while (BDisON): 
    ...
    if (choice1==2):
        ...
        while(userConnected):
            ...
            if (choiceConnected==4 and chosenUser=='3'):
                annee=int(input("De quelle année voulez vous voir le rapport ?"))

                tot_annee=0
                i=1
                while(i<13):
                    query="SELECT SUM(montant) AS somme_des_montants FROM Comptabilite WHERE MONTH(date) ={};".format(str(i))
                    cursorClient.execute(query)
                    mois = cursorClient.fetchall()
                    print("\n|Rapport du mois {}: \t".format(str(i)),mois[0][0])
                    if(mois[0][0]):
                        tot_annee+=int(mois[0][0])
                    i+=1
                

                print("\n|Rapport année: \t",tot_annee)
                input()

            if (choiceConnected==5 and chosenUser=='3'): #Check+-
                print("-")
                query="select * from PERSONNEL_ACTIF"
                cursorClient.execute(query)
                personnel = cursorClient.fetchall()
                print("Personnel actif : \n",personnel)
                input()
            ...
            if (choiceConnected==7):
                #query = "SELECT * FROM RESERVATION where NOM_PERSONNE = %s;"%CLIENT[0][0]
