...
def AjoutPersonnel(donner,connection):
    ...
    #Ajout dans PERSONNEL si c est pas un client
    if ( donner[6] != "client" ):
        if (int(donner[8])==1):
            ...
            insert_personnel_query=("INSERT INTO `CONTRAT` (`DATE_DEBUT`, `DATE_FIN`, `TYPE`, `ID_TIERS`) VALUES (now(), DATE_ADD(CURDATE(), INTERVAL 1 YEAR), '1', '{}');".format(personne_id))
            cursor.execute(insert_personnel_query)
...
#
    ...
    #cursorClient.execute(query)
#
    #query ="DELETE FROM CONTRAT WHERE ID_TIERS= %s;"%id_client
    #cursorClient.execute(query)
#
    ...
    #Anonymise les reservation passé
    query ="UPDATE RESERVATION SET NOM_PERSONNE='-',NUM_TEL='-' WHERE (select NOM from PERSONNE where ID_TIERS=%s and NOM=RESERVATION.NOM_PERSONNE);"%id_client
    cursorClient.execute(query)

    query ="UPDATE PERSONNE SET NOM='-',PRENOM='-',DATE_NAISSANCE = '0-01-01', EMAIL = '-', SEXE_='-' WHERE ID_TIERS=%s"%id_client
    cursorClient.execute(query)
